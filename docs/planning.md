...
- **X/Twitter. RESOLVED 2026-09-05 (D62) — built.** The premise expired: X moved to pay-per-use in February 2026, so access is $0.005 a post rather than a $200 subscription, and the spike became the leg. 27 handles from `config/people.yaml` (Musk excluded on volume), 90 days, originals and quote-posts, $5.35 all in. 473 posts pulled, 238 past the prefilter, 88% correctly scoring zero. Eight of 27 lab leaders posted nothing original in the window. The unique-signal question it was deferred on has a narrower answer than hoped: 352 of 409 links are quote-links to other posts, so the honest claim is about the 17 links to lab documents, 9 of which the announcements leg does not hold — including Meta's whole Muse line on `research.meta.ai`, a channel `config/sources.yaml` does not watch.
