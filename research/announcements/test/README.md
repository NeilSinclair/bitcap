...
- `inference_cost_down` — Compute and memory needed per token falls. Efficiency: the same output takes less hardware. Distillation, quantisation, sparsity, grouped-query attention and better serving stacks reduce the FLOPs and the resident bytes required per token. The DeepSeek-R1 shock of January 2025 is the canonical event. Tag this only when the *work* per token falls, not whenever a price falls. A cheaper accelerator, a discount, or margin competition lowers the price of a token without changing the hardware content of one; that is a pricing_change event, not this mechanism. It also needs a stated figure, price, or technique: product copy that calls a model "efficient" — typically in a partnership or distribution re-announcement of a model tagged elsewhere — is not evidence.
- `inference_volume_up` — Inference volume rises. More tokens served in aggregate: consumer adoption, deployment into high-volume surfaces, agentic workflows that issue many calls per task, or reasoning models that spend more compute at test time. Often the second-order offset to inference_cost_down. The direction cuts both ways: a model suspended, withdrawn or disabled for customers is this mechanism with a negative sign, not an absent tag.
- `memory_intensity_up` — Memory content per accelerator rises. Longer context windows, KV-cache pressure, mixture-of-experts weights and test-time reasoning increase HBM and NAND content per system, independently of unit accelerator volume. Tag from stated model or serving properties — a larger offered context, a deeper reasoning mode, a cache architecture — not from a customer's anecdote about feeding a model more context.
...
- `capability_jump` — Step change in model capability. A capability threshold is crossed that puts new tasks in reach -- long-horizon agents, reliable tool use, domain expert performance. Reserve it for the step itself: a frontier-class release or a first demonstration of a task class. An incremental release, a "close to X" comparison, or a partnership, usage report or opinion piece restating what an already-released model can do is NOT a jump -- that restatement is where most false tags come from.
- `agentic_workflows` — Agentic and tool-using systems displace seat-based software. The unit of work shifts from humans operating software to agents executing delegated tasks: agents calling APIs in place of seats, and agents running autonomously at scale (parallel agents, round-the-clock workflows). Both routes count -- seat displacement and the growth of agent-executed work. Tag from what the lab states or measures at scale; one customer's testimonial about their own agent architecture is not an industry shift.
...
- `export_controls` — Export controls and trade policy tighten. Government restrictions on the AI stack reaching particular parties or jurisdictions: advanced accelerators, HBM, semiconductor capital equipment, tariffs on hardware -- and equally model weights or model access withdrawn or gated under national-security or export authority. A directive that forces a lab to cut off access to a model is this mechanism, not only a chip story.
- `ai_capex_investments` — AI infrastructure capital expenditure. Capital committed to AI infrastructure -- datacentre build-outs, accelerator and chip purchases, power and energy supply. A positive sign is spending being committed or accelerated; negative is spending slowed or withdrawn because of financing cost, depreciation, utilisation or weak returns. It requires capital to be stated: an amount, a build-out, a purchase, a supply agreement. A performance result -- even on first-party silicon -- discloses no spending and is custom_silicon_substitution, not capex.
...
- `accelerator_custom_si` — Accelerators and custom silicon. AI accelerators, custom ASICs, and the controller and interconnect silicon that surrounds them. Tag when demand or supply for the silicon itself moves; software efficiency work (kernels, serving stacks) runs on accelerators without being a signal about them.
...
- `ai_compute_hosting` — AI compute hosting. Operators whose business is hosting AI compute for third parties - datacentre capacity, power and GPU fleets sold under contract (neoclouds). A hyperscale cloud distributing models or apps is hyperscale_platform, not this.
- `hyperscale_platform` — Hyperscale platforms. Cloud and platform businesses that both build and buy frontier AI. Tag when the platform relationship IS the announcement - models distributed through the platform, the platform buying or selling frontier compute. A platform named as a rollout venue or region ("pricing changes will begin rolling out in AWS") is a mention, not a signal.
- `fintech_consumer_fin` — Fintech and consumer finance. Consumer brokerage, lending and financial infrastructure.
- `health_insurance` — Health and insurance. Digital health, health insurance and clinical-stage businesses. Tag when a capability or product crosses into clinical, care or underwriting workflows. A lab doing research WITH a pharma or biotech partner is a story about the lab, not a health signal.
...
- `orchestration` — How you would build the pipeline. Techniques for structuring work around a model rather than inside it: agent scaffolding, planning and routing loops, multi-agent decomposition, context and memory management, tool-use patterns, retrieval design, caching strategy, prompt structure that materially changes behaviour. The document must describe a technique an engineering team could build from -- a design with a stated behaviour or gain. Training courses about agents, customer usage stories and product marketing that mentions "workflows" are the standing false positives here; none of them is a technique.
- `evaluation` — How you would measure or trust it. Benchmarks, eval methodology, judging and grading approaches, red-teaming and adversarial testing methods, safety evaluation technique, monitoring and drift detection, and honest accounts of what an eval does not measure. Safety technique is folded in here on purpose rather than given its own tag. A jailbreak defence, a cyber safeguard and a coding benchmark are all answers to "how would we know whether this is good enough to ship". Split it out only if the human-versus-system disagreements justify it. Tag it even when evaluation is not the document's subject. A benchmark used to support a hardware claim, a red-teaming effort disclosed in a statement, a preparedness threshold applied in a release -- if an engineer could learn measurement practice from the quote, it belongs here. This is the most under-tagged practice: it was missed in four of twenty gold articles, every time because it was secondary to the story.
