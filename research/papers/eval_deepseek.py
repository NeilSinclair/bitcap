"""Compare LLM author-list extraction against the deterministic DeepSeek parser.

Mirrors research/eval_byline.py, but for DeepSeek's shape: hundreds of authors per
paper, grouped by contribution role, with departures marked. The deterministic
output is the reference, not truth - disagreements are printed for adjudication.

Usage:
    python research/eval_deepseek.py
"""
...
PROMPT = ROOT / "prompts" / "byline_extraction" / "deepseek_v1.md"
REGISTER = ROOT / "research" / "docs" / "deepseek_contributors.json"
CACHE = ROOT / "research" / "docs" / "deepseek_cache"
OUT = ROOT / "research" / "docs" / "deepseek_eval.json"
COST = ROOT / "research" / "docs" / "deepseek_llm_cost.json"
    ...
    "properties": {
        "authors": {
            ...
            "items": {
                ...
                "required": ["name", "role", "departed"],
                "additionalProperties": False,
            },
        },
        "departure_marker_meaning": {"type": ["string", "null"]},
        "order_meaningful": {"type": "boolean"},
        "no_author_list": {"type": "boolean"},
    },
    "required": ["authors", "departure_marker_meaning", "order_meaningful", "no_author_list"],
    "additionalProperties": False,
}
...
def prepare(raw: str) -> tuple[str, bool]:
    ...
    if len(body) <= HTML_BUDGET:
        return body, False
    anchor = body.rfind("Author List")
    if anchor < 0:
        return body[-HTML_BUDGET:], True
    start = max(0, anchor - 2_000)
    return body[start : start + HTML_BUDGET], True
...
def compare(paper: dict, pred: dict) -> dict:
    ...
    for k in shared:
        ...
        if bool(g["departed"]) != bool(p["departed"]):
            dep_diffs.append({"name": g["name"], "gold": g["departed"], "llm": p["departed"]})
    ...
    return {
        ...
        "gold_departed": sum(1 for a in paper["authors"] if a["departed"]),
        "llm_departed": sum(1 for a in pred.get("authors", []) if a["departed"]),
        "order_meaningful_llm": pred.get("order_meaningful"),
        "marker_meaning_llm": pred.get("departure_marker_meaning"),
    }
...
def main() -> None:
    ...
    papers = [p for p in register["papers"] if p["roles_source"] == "author_list"]
    ...
    summary = {
        ...
        "departed_disagreements": sum(len(r["departed_diffs"]) for r in rows),
    ...
    for r in rows:
        ...
        print(f"    departed gold={r['gold_departed']} llm={r['llm_departed']}"
              f"  disagreements={len(r['departed_diffs'])}")
        print(f"    role disagreements: {len(r['role_diffs'])}")
        print(f"    marker read as: {r['marker_meaning_llm']}")
